<template>
  <div class="h-screen flex flex-col overflow-hidden">
    ...
    <div class="flex items-center border-b border-gray-200 px-4 py-2 gap-2">
      <span class="font-bold text-lg">在线Markdown笔记</span>
      <div class="flex-1 flex gap-1">
        <div
          v-for="tab in openTabs"
          :key="tab.id"
          class="px-3 py-1 rounded cursor-pointer flex items-center gap-1"
      ...
      <button class="px-3 py-1 bg-gray-200 rounded" @click="showSetting=true">AI配置</button>
    </div>
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <div class="w-60 border-r border-gray-200 flex flex-col">
        ...
        <div class="flex-1 overflow-auto">
          <div
            ...
            :key="note.id"
            class="p-2 border-b cursor-pointer hover:bg-gray-50 flex justify-between"
            :class="activeId===note.id ? 'bg-blue-100' : ''"
            @click="openNote(note.id)"
          >
            <span>{{ note.title }}</span>
            <span @click.stop="delNote(note.id)" class="text-red-400">删</span>
          </div>
        </div>
      </div>
      ...
      <div class="flex-1 flex flex-col">
        <div v-if="currentNote" class="p-2 border-b flex gap-3 items-center">
          <input
        ...
        <MdEditor v-if="currentNote" v-model="currentNote.content" @change="updateNote(currentNote.id,{content:currentNote.content})"/>
        <div v-else class="flex items-center justify-center h-full text-gray-400">请新建/选择一条笔记</div>
      </div>
    </div>
